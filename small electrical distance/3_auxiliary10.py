...
F1_2 = (-11382104.842903600000-FU_1)/(FN_1-FU_1)
F1_3 = (382189345.774046000000-FU_1)/(FN_1-FU_1)

F2_1 = (-7410100.992565510000-FU_2)/(FN_2-FU_2)
F2_2 = (FU_2-FU_2)/(FN_2-FU_2)
F2_3 = (16078795.267308400000-FU_2)/(FN_2-FU_2)

F3_1 = (136.367463382228-FU_3)/(FN_3-FU_3)
F3_2 = (129.229932956956-FU_3)/(FN_3-FU_3)
...
np.save('3w_train_ws_woPC_OLTC_'+str(len(f_train))+'_extreme'+'.npy', w_train)
np.save('3f_train_ws_woPC_OLTC_'+str(len(f_train))+'_extreme'+'.npy', f_train)
...
fig.savefig('Pareto_ws_woPC_F1_OLTC_'+str(len(f_train))+'_extreme')
plt.show()
# F2
fig = plt.figure()
ax = Axes3D(fig)
...
ax.set_xlabel('w1', fontdict={'size': 15, 'color': 'red'})
ax.set_title('Points before Pareto check')
fig.savefig('Pareto_ws_woPC_F2_OLTC_'+str(len(f_train))+'_extreme')
plt.show()
# F3
fig = plt.figure()
ax = Axes3D(fig)
...
ax.set_xlabel('w1', fontdict={'size': 15, 'color': 'red'})
ax.set_title('Points before Pareto check')
fig.savefig('Pareto_ws_woPC_F3_OLTC_'+str(len(f_train))+'_extreme')
plt.show()
# F2(F1,F3)
fig = plt.figure()
ax = Axes3D(fig)
...
ax.set_title('Points before Pareto check')
fig.savefig('Pareto_ws_woPC_F123_OLTC_'+str(len(f_train))+'_extreme')
plt.show()
...
w_train = np.vstack((w_train, w_bound))
f_bound = [
    [F1_1,F2_1,F3_1],
    [F1_2,F2_2,F3_2],
    [F1_3,F2_3,F3_3]
...
np.save('3f_train_ws_wPC_OLTC_'+str(len(f_train))+'_extreme'+'.npy', f_train)
np.save('3w_train_ws_wPC_OLTC_'+str(len(f_train))+'_extreme'+'.npy', w_train)

print('length after ParetoCheck:',len(f_train))
...
ax.set_xlabel('F1', fontdict={'size': 15, 'color': 'red'})
ax.set_title('Points before Pareto check')
fig.savefig('Pareto_ws_wPC_F123_OLTC_'+str(len(f_train))+'_extreme')
plt.show()
...
fig.savefig('Pareto_weight_ws_wPC_OLTC_'+str(len(f_train))+'_extreme')
plt.show()
