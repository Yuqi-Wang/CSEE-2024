...
def main_fun(obj_index,w1,w2,ite,tol,ite_tol):
    ...
    model.addConstrs((omig_ess_ch[b,i,j] + omig_ess_dch[b,i,j] <= ess_w[b]\
        for b in range(ess_num) for i in range(N) for j in range(24)), name='c1')
    ...
    for i in np.arange(1,Busnum):
        model.addConstrs((u_bus[i,j,t] == u_bus[i_fa[i]-1,j,t] - 2*(p_line[i-1,j,t]*line_r[i]\
            + q_line[i-1,j,t]*line_x[i]) + i_line[i-1,j,t]*(np.square(line_r[i])+np.square(line_x[i]))\
                for j in range(N) for t in range(24)), name = 'c9')
    ...
    if obj_index == 1:
        ...
        model.setObjective(w1*(F1-FU_1)/(FN_1-FU_1) + w2*(F2-FU_2)/(FN_2-FU_2) + (1-w1-w2)*(F3-FU_3)/(FN_3-FU_3))
    ...
    if model.Status == 2:
        ...
        print('This is the '+str(ite_tol)+'th iteration.')
        print(str(ite/tol*100)+'%, '+str(ite)+' gets the sulotion!!!')

        filename = 'DataFile'+'//result_obj'+str(obj_index)+'_w1_'+str(w1)+'_w2_'+str(w2)+'_pv'+str(P_pv_max)+'.npz'
        ...
        # print('scalarized objectives: ',np.array([F1,F3,F2]))

        return np.array([F1,F2,F3])

    else:
        print(str(ite/tol*100)+'%, '+str(ite)+' can not get the sulotion...')
