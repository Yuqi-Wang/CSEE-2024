...
line_r = scipy.io.loadmat('line_r.mat')['line_r']/z_pu * RX# the i th element is the R in the line where bus i is its son bus, 读取出的单位是Ω，除以z_pu化成标幺值
line_x = scipy.io.loadmat('line_x.mat')['line_x']/z_pu * RX# the i th element is the X in the line where bus i is its son bus, 读取出的单位是Ω，除以z_pu化成标幺值
...
if model.Status == 2:
    ...
    filename = 'result_obj'+str(obj_index)+'_w1_'+str(w1)+'_w2_'+str(w2)+'_pv'+str(P_pv_max)+'_RX_'+str(RX)+'.npz'
