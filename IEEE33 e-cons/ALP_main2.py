...
if __name__ == '__main__':
    ...
    f_train = np.load('3f_train_ws_woPC_OLTC_'+str(initial_num)+'.npy')
    w_train = np.load('3w_train_ws_woPC_OLTC_'+str(initial_num)+'.npy') # all the three weights are given in original file

    # Load the testing data
    f_testing = np.load('3f_test_ws_OLTC_'+str(testing_num)+'.npy')
    w_testing = np.load('3w_test_ws_OLTC_'+str(testing_num)+'.npy')
    ...
    model.add(tf.keras.layers.Dense(368, activation='relu', input_shape=(w_train.shape[1],)))
    ...
    while (f1_diff > diff_max) or (f2_diff > diff_max) or (f3_diff > diff_max):
    # while ite < num_iterations:
        ...
        np.save('w_test_tol_iteration_'+str(initial_num)+'_iter_'+str(num_iterations)+'_MSE_'+str(MSE)+'_MAE_'+str(MAE)+'_diff_'+str(diff)+'.npy',w_test_tol)
